...
function sb_delete_user($user_id) {
    ...
    if (isset($livesmart) && isset($livesmart['livesmart_url'])) {
        $result = sb_db_get('SELECT email FROM sb_users WHERE id = "' . $user_id . '" LIMIT 1');
...
<div class="sb-editor">
    ...
    <div class="sb-bar">
        <div class="sb-bar-icons">
            ...
            //LiveSmart code change start. Code added to integrate LiveSmart check user.
            $livesmart = sb_get_setting('livesmart-settings');
            if (!$admin && isset($livesmart) && isset($livesmart['livesmart_url'])) {
                $lsRepUrl = (substr($livesmart['livesmart_url'], -1) == '/') ? $livesmart['livesmart_url'] : $livesmart['livesmart_url'] . '/';
                $active_user = sb_get_active_user();
                ?>
                <div data-sb-tooltip="<?php sb_e('Start Video') ?>">
                ...
                <script>
                    $("#livesmart_startvideo").click(function(event) {
                        ...
                        var str = {};
                        str.lsRepUrl = '<?php echo $lsRepUrl;?>';
                        str.names = 'Chat agent';
                        str.visitorName = '<?php echo $active_user['first_name'] . ' ' . $active_user['last_name']; ?>';
                        str.agentId = '<?php echo $active_user['email']; ?>';
                        var encodedString = window.btoa(unescape(encodeURIComponent(JSON.stringify(str))));
                        encodedString = encodedString.split('=').join('');
                        visitorUrl = str.lsRepUrl + 'pages/r.html?room=' + roomId + '&p=' + encodedString;
                        str.isAdmin = 1;
                        ...
                        chat_textarea.val('<?php sb_e('Request for a video chat') ?> ' + agentUrl);
