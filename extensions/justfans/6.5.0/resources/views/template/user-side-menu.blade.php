<nav class="sidebar {{(Cookie::get('app_theme') == null ? (getSetting('site.default_user_theme') == 'dark' ? '' : 'light') : (Cookie::get('app_theme') == 'dark' ? '' : 'light'))}}">
    ...
    <ul class="list-unstyled menu-elements p-0">
        @if(GenericHelper::isEmailEnforcedAndValidated())
            ...
            @if(getSetting('livesmart.url'))
            <li class="{{Route::currentRouteName() == 'my.livesmart' ? 'active' : ''}}">
                <a class="scroll-link d-flex align-items-center" href="{{route('my.livesmart')}}">
                    @include('elements.icon',['icon'=>'videocam','variant'=>'medium','centered'=>false,'classes'=>'mr-2'])
                    {{__('LiveSmart Dashboard')}}</a>
            </li>
            @endif
