                ...
                <div>
                    @if(!Auth::check() || Auth::user()->id !== $user->id)
                        <div class="d-flex flex-row">
                            ...
                            @if(getSetting('livesmart.url') && Auth::check())
                                <span class="p-pill ml-2 pointer-cursor" id="livesmart_span" title="{{__('Start Video Call')}}" data-receiverid="{{$user->id}}" data-message="{{__('Request for a video chat')}}" data-visitorname="{{Auth::user()->name}}" data-username="{{$user->username}}" data-agentname="{{$user->name}}" onclick="startVideoChat()">
                                    @include('elements.icon',['icon'=>'videocam'])
                                </span>
                            @endif
